...
function MenuItem({
  ...
  const handleMouseEnter = (ev: React.MouseEvent<HTMLAnchorElement> | React.TouchEvent<HTMLAnchorElement>) => {
  ...
  const handleMouseLeave = (ev: React.MouseEvent<HTMLAnchorElement> | React.TouchEvent<HTMLAnchorElement>) => {
    if (!itemRef.current || !marqueeRef.current || !marqueeInnerRef.current) return;
    const rect = itemRef.current.getBoundingClientRect();
    let clientX = 0;
    let clientY = 0;
